...
def _candidate_node_dirs(env: dict[str, str]) -> list[Path]:
    candidates: list[Path] = []
    hermes_home = env.get("HERMES_HOME")
    if hermes_home:
        candidates.append(Path(hermes_home).expanduser() / "node" / "bin")
    ...
    if nvm_versions.is_dir():
        candidates.extend(
            sorted(
                (path / "bin" for path in nvm_versions.iterdir() if path.is_dir()),
                reverse=True,
            )
        )
...
def _path_with_node(env: dict[str, str]) -> str:
    ...
    for candidate in _candidate_node_dirs(env):
        node_path = candidate / "node"
        if node_path.is_file() and os.access(node_path, os.X_OK):
            return os.pathsep.join([str(candidate), *existing_parts])
...
def contract_json(args: dict[str, Any]) -> str | None:
    contract = args.get("contract") or args.get("answer_contract") or args.get("answerContract")
    if contract is None:
        return None
...
def runner_command(args: dict[str, Any]) -> tuple[list[str], str | None]:
    ...
    if mode in {"validate_contract", "run_contract"}:
        stdin = contract_json(args)
        if not stdin:
            raise ValueError(f"mode='{mode}' requires contract JSON.")
        script = (
            "scripts/validate-contract.ts"
            if mode == "validate_contract"
            else "scripts/run-contract.ts"
        )
        command = [
            "node",
            "--import",
        ...
        if dry_run and mode == "run_contract":
            command.append("--dry-run")
        return command, stdin
    ...
    raise ValueError(
        ...
        "posthog_ad_hoc, validate_contract, run_contract, "
        "source_change_plan, source_change_scope_check, "
