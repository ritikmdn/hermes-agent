...
from .slack_personality import (
    compose_chandler_slack_response,
    sanitize_default_slack_text,
    should_append_dashboard_link,
)
...
def has_answer_artifact(value: Any) -> bool:
    if isinstance(value, dict):
        artifact = value.get("answerArtifact")
        if isinstance(artifact, dict) and artifact.get("version") == "answer-artifact.v1":
            return True
        return any(has_answer_artifact(nested) for nested in value.values())

    if isinstance(value, list):
        return any(has_answer_artifact(item) for item in value)
...
def safe_payload_summary(payload: Any) -> dict[str, Any]:
    ...
    source_result = payload.get("sourceResult")
    if not isinstance(source_result, dict):
        source_result = {}

    route = payload.get("route") or source_result.get("route")
    shortcut = payload.get("shortcut") or source_result.get("shortcut")
    if route:
        summary["route"] = route
    if shortcut:
        summary["shortcut"] = shortcut
    if has_answer_artifact(payload):
        summary["answerArtifact"] = True

    nested_payload = payload.get("payload")
    source_payload = source_result.get("payload")
    if isinstance(nested_payload, dict):
        row_payload = nested_payload
    elif isinstance(source_payload, dict):
        row_payload = source_payload
    elif source_result:
        row_payload = source_result
    else:
        row_payload = payload
...
def compact_answer_question_payload(payload: Any) -> Any:
    ...
    shortcut = payload.get("shortcut") if isinstance(payload.get("shortcut"), str) else None
    nested_payload = payload.get("payload")
    ...
    if isinstance(slack_text, str) and slack_text.strip():
        ...
        personalized_text = compose_chandler_slack_response(
            answer_payload,
            route=route,
            shortcut=shortcut,
        )
        compact_payload["slackText"] = compact_slack_text(
            personalized_text or sanitize_default_slack_text(slack_text)
        )
    ...
    result_type = compact_payload.get("resultType")
    if result_type is None and isinstance(compact_payload.get("metadata"), dict):
        result_type = compact_payload["metadata"].get("resultType")
    rows = answer_payload.get("rows")
    if result_type == "kpi" and isinstance(rows, list) and len(rows) <= 3:
        compact_payload["rows"] = [
            row for row in rows if isinstance(row, dict)
        ]

    answer_artifact = answer_payload.get("answerArtifact")
    if isinstance(answer_artifact, dict):
        compact_payload["answerArtifact"] = answer_artifact
...
def direct_final_response_for_answer_payload(payload: Any) -> str | None:
    ...
    answer_artifact = answer_payload.get("answerArtifact")
    if not (
        isinstance(answer_artifact, dict)
        and answer_artifact.get("version") == "answer-artifact.v1"
    ):
        return None
    ...
    final_text = compact_direct_final_slack_text(
        personalized_text or sanitize_default_slack_text(slack_text)
    )
    ...
    if (
        dashboard_line
        and "dashboard:" not in final_text.lower()
        and should_append_dashboard_link(answer_payload, route=route, shortcut=shortcut)
    ):
