...
- Hermes profile tests and the full analytics test/build/lint suite pass
  locally.
- The `elixir-analytics` distribution ships `elixir-analytics-runner` as a
  profile-local plugin under `plugins/`, matching upstream Hermes discovery.
...
- Chandler overlay draft PR #1 is open in `ritikmdn/hermes-agent` from
  `codex-chandler-overlay-origin-main`. A true upstream-based sync branch
  `codex-chandler-upstream-sync` also exists locally on top of
  `NousResearch/hermes-agent` `main`, with patch and bundle artifacts in
  `/private/tmp`. Opening a public NousResearch upstream PR remains an explicit
  product/privacy decision because the profile is Elixir-specific.
...
| 13 | Hermes upstream sync | The profile distribution is pushed or PR'd upstream. | Draft fork PR open; upstream-based patch/bundle prepared |
...
1. Hermes profile package: distribution sync, profile-local
...
Current evidence: draft PR #1 is open in the `ritikmdn/hermes-agent` fork from
`codex-chandler-overlay-origin-main`. The true upstream-based branch
`codex-chandler-upstream-sync` is one commit ahead of current
`NousResearch/hermes-agent` `main`; patch and bundle artifacts are available in
`/private/tmp`; focused verification passed 82 Hermes Chandler tests plus
release packaging, ruff, compileall, and whitespace checks.
...
- Decide whether to submit the prepared Chandler overlay to public
  `NousResearch/hermes-agent`, keep it fork/private, or continue using the
  patch/bundle sync handoff.
