...
from hermes_cli.profile_distribution import DistributionManifest, install_distribution
...
def test_elixir_analytics_distribution_manifest_is_installable():
    ...
    assert "plugins" in manifest.owned_paths()
...
def test_elixir_analytics_distribution_ships_runner_plugin():
    plugin_dir = DIST_DIR / "plugins" / "elixir-analytics-runner"
...
def test_installed_elixir_analytics_profile_exposes_runner_plugin(
    tmp_path,
    monkeypatch,
):
    ...
    plan = install_distribution(str(DIST_DIR), name="elixir-analytics-test")
    ...
    plugins = {entry["key"]: entry for entry in manager.list_plugins()}
    assert plugins["elixir-analytics-runner"]["enabled"] is True
